...
def _session_return_pct(
    ...
    opening = _exact_bar(connection, ticker, session_open)
    latest = _exact_bar(connection, ticker, through)
...
def _signal_pullback(
    ...
    current = _exact_bar(connection, ticker, ts)
    previous = _exact_bar(connection, ticker, ts - window * 60)
    if current is None:
        return None
    price = float(current["close"])
    movement = (
    ...
    high, low = _running_extremes(connection, ticker, session_open, ts)
    running_high = high if high is not None else price
    running_low = low if low is not None else price
    distance = (
